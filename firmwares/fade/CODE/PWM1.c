...
**             microseconds            : 3914       0
**             milliseconds            : 4          0
**             seconds (real)          : 0.003914373089 0.0
**
**     Contents  :
...
static void SetRatio(void)
{
  if (RatioStore == 0xFFFFU) {         /* Duty = 100%? */
    ...
    TPM2C0V = (word)(RatioStore >> 0); /* Calculate new compare value according to the given ratio */
  }
}
...
**                      (0 to 3914 us in high speed mode)
**     Returns     :
**         ---             - Error code, possible codes:
**                           ERR_OK - OK
**                           ERR_SPEED - This device does not work in
...
byte PWM1_SetDutyUS(word Time)
{
  ...
  if (Time > 0x0F4AU) {                /* Is the given value out of range? */
    return ERR_RANGE;                  /* If yes then error */
  }
  PE_Timer_LngMul((dword)Time, 0x10BE0DEDLU, &rtval); /* Multiply given value and High speed CPU mode coefficient */
...
byte PWM1_SetDutyMS(word Time)
{
  ...
  PE_Timer_LngMul((dword)Time, 0x41666666LU, &rtval); /* Multiply given value and High speed CPU mode coefficient */
...
void PWM1_Init(void)
{
  ...
  setReg8(TPM2C0SC, 0x3C);             /* Set up PWM mode with output signal level low */ 
  RatioStore = 0x00U;                  /* Store initial value of the ratio */
  /* TPM2MOD: BIT15=1,BIT14=1,BIT13=1,BIT12=1,BIT11=1,BIT10=1,BIT9=1,BIT8=1,BIT7=1,BIT6=1,BIT5=1,BIT4=1,BIT3=1,BIT2=1,BIT1=1,BIT0=1 */
  setReg16(TPM2MOD, 0xFFFFU);          /* Set modulo register */ 
