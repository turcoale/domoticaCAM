...
byte Cap1_Enable(void)
{
  if (!EnUser) {                       /* Is the device disabled by user? */
    ...
    /* TPM1SC: TOF=0 */
    clrReg8Bits(TPM1SC, 0x80);         /* Reset overflow interrupt request flag */ 
    /* TPM1SC: TOIE=1 */
    setReg8Bits(TPM1SC, 0x40);         /* Enable overflow interrupt */ 
    /* TPM1SC: CLKSB=1,CLKSA=0 */
    clrSetReg8Bits(TPM1SC, 0x08, 0x10); /* Run counter */ 
  }
  return ERR_OK;                       /* OK */
}
...
byte Cap1_Disable(void)
{
  if (EnUser) {                        /* Is the device enabled by user? */
    ...
    /* TPM1SC: TOIE=0 */
    clrReg8Bits(TPM1SC, 0x40);         /* Disable overflow interrupt */ 
  }
  return ERR_OK;                       /* OK */
}
...
void Cap1_Init(void)
{
  ...
  clrSetReg8Bits(TPM1SC, 0x03, 0x04);  /* Set prescaler register */ 
  /* TPM1C0SC: CH0F=0,CH0IE=1,MS0B=0,MS0A=0,ELS0B=1,ELS0A=1,??=0,??=0 */
  setReg8(TPM1C0SC, 0x4C);             /* Enable both interrupt and capture function */ 
  /* TPM1SC: TOF=0 */
  clrReg8Bits(TPM1SC, 0x80);           /* Reset overflow interrupt request flag */ 
